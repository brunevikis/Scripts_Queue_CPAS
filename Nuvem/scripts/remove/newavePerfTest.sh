...
hostfile="/home/producao/PrevisaoPLD/cpas_ctl_common/hosts/test.hosts.mpi"
cores=$1
...
echo "/opt/aplicacoes/newave/bin/" >> caso.dat


echo -e "\n"
echo "-----------[ Convertendo nome dos arquivos ] --------------"
echo /opt/aplicacoes/newave/bin/ConverteNomesArquivos
/opt/aplicacoes/newave/bin/ConverteNomesArquivos;
...
if [ $tipoExec -eq 1 ]; then
	...
	echo "-----------[ Limpando sujeira ] --------------"
		...
		#echo /opt/aplicacoes/newave/bin/newave${ver}_L
		#/opt/aplicacoes/newave/bin/newave${ver}_L;
		...
		if [ $tipoSim -eq 1 -o $tipoSim -eq 2 ]; then
			...
			#for xx in `tail -n+2 /etc/hosts.mpi | grep -v '#'`; do ssh $xx "mkdir -p $work_dir"; done      
			...
			#/usr/local/bin/mpiexec -f /etc/hosts.mpi -n ${cores} /opt/aplicacoes/newave/bin/newave${ver}_L;
            echo "/opt/aplicacoes/mpich-3.1.4/install/bin/mpiexec -f ${hostfile} -n ${cores} /opt/aplicacoes/newave/bin/newave${ver}_L"
            /opt/aplicacoes/mpich-3.1.4/install/bin/mpiexec -f "${hostfile}" -n ${cores} /opt/aplicacoes/newave/bin/newave${ver}_L;
            ...
            wait;
            rm -rf ${work_dir};
			#for xx in `tail -n+2 /etc/hosts.mpi | grep -v '#'`; do ssh $xx "rm -rf $work_dir"; done
