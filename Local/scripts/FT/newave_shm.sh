...
if [ $tipoExec -eq 1 ]; then
    ...
    if [ $tipoSim -eq 1 -o $tipoSim -eq 2 ]; then
        ...
        origin_dir=$(pwd)
        mkdir /dev/shm/newave
        #definir diretorio para processamento /dev/shm/newave/$(date +%Y%m%d%H%M%s%N)
        ...
        cd "${work_dir}" || exit 2
        echo -e "\n-----------[ Executando o Newave ] --------------"
        echo "/home/producao/mpich2/bin/mpiexec -n ${cores} /home/producao/PrevisaoPLD/shared/install/newave/newave_${ver}/Executaveis/newave${ver}_L"
        /home/producao/mpich2/bin/mpiexec -n ${cores} /home/producao/PrevisaoPLD/shared/install/newave/newave_${ver}/Executaveis/newave${ver}_L;
        ...
        { zip -q etc cortes-*.dat cortese-*.dat alertainv_00* avl_* sgterm* newave_00*.log fpha_0*.svc eng*.dat && rm -f cortes-*.dat cortese-*.dat alertainv_00* avl_* sgterm* newave_00*.log fpha_0*.svc eng*.dat; } &
        wait;
