...
agendaDCGNL(){
    ...
    cmd="/home/producao/PrevisaoPLD/enercore_ctl_common/scripts/GNL/DADGNL_decomp32.0.1Viab_earDCsqn_ANA_NW30.0.4.sh"
...
if [ -d $ano$mes ]; then
  ...
  if [ -f "cortesh.dat" ]; then
    ...
    #if [ -d ../$anoN$mesN ]; then
    #  curl -s --get --data-urlencode "path=$encadPath" --data-urlencode "date=$ano$mes" "http://10.206.194.210/api/encad/encad_dcsqn_restr" &
    #fi
