...
next="$(echo $dir|cut -c5-15)$n$(echo $dir|cut -c17-)"
echo $INICIO
echo ""
echo "$dir"
echo "$comando"
...
then
    ...
    echo /home/compass/sacompass/previsaopld/cpas_ctl_common/scripts/buscaEarm_dc_dc.sh "$(pwd)" "$nrev/$next"
    /home/compass/sacompass/previsaopld/cpas_ctl_common/scripts/buscaEarm_dc_dc.sh "$(pwd)" "$nrev/$next"
    echo "AGENDAR NOVA EXECUCAO"
    dt=$(date +%Y%m%d%H%M%S)
    ord=$(( 10 ))
    usr="encad"
    fn="/home/compass/queuectl/waiting_prevs/$next"
    cmd="$0"
