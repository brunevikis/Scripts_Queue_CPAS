...
rm -f debug_*.rv?;
rm -f cad*.rv?;
mv indices.csv indices.cs_
mv polinjus.csv polinjus.cs_
rm -f ./*.csv;
mv indices.cs_ indices.csv
mv polinjus.cs_ polinjus.csv
rm -f qsinp*;
rm -f qsout*;
rm -f relato.rv*;
rm -f sumario.rv*;
...
echo "                  *fpha[._]* relato2.* mapcut.* custos.* fcfnwn.* *.csv;";
