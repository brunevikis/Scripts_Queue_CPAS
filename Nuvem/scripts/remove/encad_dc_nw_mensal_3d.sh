...
colectEARMFinal(){
	...
	ln=$( grep "EARMF" consulta.nwd )
	lnM=$( grep "EARMx" consulta.nwd )
    ...
    for reeI in ${!rees[*]}; do
		...
		if float_cond "$earmmax[$ree] > 0"
		  earmfp[$ree]=$( echo " scale=2 ; (${earmf[$ree]}/${earmmax[$ree]})*100 " | bc -l)
		fi
...
colectEARMInicialNext(){
    ...
    for reeI in ${!rees[*]}; do
        ...
        if [ "${earmipNext[$ree]}" = "0" ]; then      
                ...
                earmmaxNext[$ree]=$( echo " scale=2 ; (${earmiNext[$ree]}/${earmipNext[$ree]})*100 " | bc -l)
            fi		
        done
...
setEARMNewdespNext(){
    ...
    for reeI in ${!rees[*]}; do
		...
		_vI=$( echo "scale=1 ;  (( $_vI * ${earmmaxNext[${rees[$reeI]}]} ) / 100)" | tr - 0 | bc -l )
