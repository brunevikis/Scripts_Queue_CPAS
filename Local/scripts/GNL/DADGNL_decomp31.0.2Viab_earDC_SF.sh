...
agendaDC(){
    ...
    cmd="/home/producao/PrevisaoPLD/enercore_ctl_common/scripts/FT/decomp31.0.2Viab.sh"
...
AgendaProximaIteracao(){
    ...
    cmd="/home/producao/PrevisaoPLD/enercore_ctl_common/scripts/GNL/encad_dc_nw_mensal_3_29_earDC_SF.sh $1"
...
if($5==156){v[295]=$12} #3M
if($5==162){v[294]=$12} #Queimad
if($5==155){v[308]=$12} #Retiro Baixo
if($5==148){v[298]=$12} #Irape
if($5==251){v[291]=$12/.55;if(v[291]>100){v[291]=100}} #SM
if($5==257){v[303]=$12} #Peixe
if($5==57){v[319]=$12} #Maua
};next}
...
' temp "../../$anoN$mesN/confhd.bkp" > "$confhd"
    ...
    mv "${modif}" "${modif}.bkp"
    awk -v "n=$n"  -v "y=$anoN" -v "m=$mesN" -f "/home/producao/PrevisaoPLD/enercore_ctl_common/awk/2024/SF+Paranapanema/modif.awk" temp "${modif}.bkp" >> "${modif}" || exit 111
    re=$(find "../../$anoN$mesN" -iname 're.dat')
    mv "$re" "$re.bkp"
    awk -v "n=$n" -v "y=$anoN" -v "m=$mesN" -f "/home/producao/PrevisaoPLD/enercore_ctl_common/awk/2024/SF+Paranapanema/re.awk" temp "$re.bkp" > "$re"
    dcPaths=$( ls ../.. | grep -E '^20[0-9]{4}' -v )
      for dc in $dcPaths ; do
...
' temp "../../$dc/$anoN$mesN/dadger.bkp" > "$dadger.tmp"
          awk -v n=$n -v m=$mesN -v nn=$nn -f "/home/producao/PrevisaoPLD/enercore_ctl_common/awk/2024/SF+Paranapanema/dadger.awk"  temp "$dadger.tmp" > "$dadger"
          rm "$dadger.tmp"
        fi
      done
    rm temp
