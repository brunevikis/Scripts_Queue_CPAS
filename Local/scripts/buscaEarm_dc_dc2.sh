...
replaceUH()
{
    ...
    do
        ...
        _uh=$( echo ${array[5]} | bc )
        _vFin=$( echo ${array[12]} | tr - 0 | bc -l )
        volFin[$_uh]=$_vFin
        
    done <<< "$( unzip -p "$origem/csv.zip" dec_oper_usih.csv | grep -E '^\s*('${periodofinal}')\s*;\s*(\1)\s*;\s*1\s*;\s*-\s')"
    
    [ -z ${volFin[309]} ] && volFin[309]="0"
    [ -z ${volFin[260]} ] && volFin[260]="0"
