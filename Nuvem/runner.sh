...
brr=;
  ...
  for temp in $commav; do
	echo $temp
    t+=("$(grep "$temp;" config | cut -b 1 | grep -v ^# -m 1)")
  done
  echo "Vms Disponiveis"
  echo ${t[@]}
  tmp=$t
  ...
  do
    ...
    n=$(printf '%s\n' ${t[@]} | grep -n -m 1 $x| cut -c 1-2)
	
	if expr "$n" + 0  > /dev/null 2>&1 && [ "$n" -gt 0 ]; then
		echo "entrou no if =" $n
	else
...
then
    ...
    then
      ...
      elif [ "$CLUSTER" == "AZCPSPLDDESSEMV7A" ]
      then
	  ...
	  elif [ "$CLUSTER" == "AZCPSRELATORIO-R" ]
      then
		( { ssh compass@$CLUSTER " echo \"$PASSWORD\" | sudo -E -S $CMD " > $DIR/run_log/${commandname}.run 2>&1; echo "EXITCODE=$?" >> $rcommand; cd $DIR; sleep 2; echo "ETIME=$(TZ=":GMT+3" date +%Y-%m-%d\ %T)" >> $rcommand; rm $rcommand; } & { echo "PID=$!" >> $rcommand; echo "STIME=$(TZ=":GMT+3" date +%Y-%m-%d\ %T)" >> $rcommand; echo "$(TZ=":GMT+3" date +%Y-%m-%d\ %T) : PID=$!"; } );
	  else
