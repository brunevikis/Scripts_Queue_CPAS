...
AgendaProximaIteracao(){
    ...
    cmd="/home/producao/PrevisaoPLD/enercore_ctl_common/scripts/FT/GNL/encad_dc_nw_mensal_3_28.16.4_micropen_hibr_earDC_regrasANA.sh $1"
...
' temp "../../$anoN$mesN/confhd.bkp" > "$confhd"
    ...
    mv "${modif}" "${modif}.bkp"
    awk -v "n=$n"  -v "y=$anoN" -v "m=$mesN" -f "/home/producao/PrevisaoPLD/enercore_ctl_common/awk/2024/SF+Paranapanem+Grande+Paranaiba/modif_hibrido.awk" temp "${modif}.bkp" >> "${modif}" || exit 111
...
' temp "../../$dc/$anoN$mesN/dadger.bkp" > "$dadger.tmp"
          awk -v n=$n -v m=$mesN -v nn=$nn -f "/home/producao/PrevisaoPLD/enercore_ctl_common/awk/2024/SF+Paranapanem+Grande+Paranaiba/dadger_hibrido.awk"  temp "$dadger.tmp" > "$dadger"
          rm "$dadger.tmp"
        fi
      done
    rm temp
