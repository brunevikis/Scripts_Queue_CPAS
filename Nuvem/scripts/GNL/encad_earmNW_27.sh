...
agendaDCGNL(){
    ...
    usr="encad"
    fn="/home/compass/queuectl/queue/encadeado_DC${ano}${mes}_${dt}"
    cmd="/home/compass/sacompass/previsaopld/cpas_ctl_common/scripts/GNL/DADGNL_decomp301ViabREE.sh"
...
setEARMDecompNext(){
    ...
    for sis in {1..4}; do
        ...
        for ree in ${rees[@]}; do
            ...
            if [[ ${reeSis[$ree]} -eq $sis ]]; then      
                ...
                if [[ $ree -eq 5 ]]; then
					earmNp[$ree]=$volumeFinalRee
                elif [ $( echo "$volumeFinalRee == 0 " | bc -l ) = "1" ]; then
                    earmNp[$ree]=0
                else
					earmNp[$ree]=$( echo "$volumePO / $volumeFinalMercado" | bc -l )
                fi
            fi
        done
    done
	...
	for dc in $dcPaths ; do
		if [ -d "$dc/$2" ]; then
            dadger=$(find DCGNL_100_100_100_100/$2/ -iname dadger.rv?)
			...
			-v bmonte="${earmNp[8]}" \
			-v manap="${earmNp[9]}" \
			'/^UH/{if($3==1){$4=sudeste*$4};if($3==2){$4=sul*$4};if($3==3){$4=nordeste*$4};if($3==4){$4=norte*$4};if($3==5){$4=itaipu};if($3==6){$4=madeira*$4};if($3==7){$4=tpires*$4};if($3==8){$4=bmonte*$4};if($3==9){$4=manap*$4};if($3==10){$4=parana*$4};if($3==11){$4=iguacu*$4};if($3==12){$4=prnpanema*$4};if($2==73){$4=100};{$4>100?r=100:r=$4;printf "%s %4s %3s %12.2f %15s %28s\n", $1,$2,$3,r,$5,$6}} !/^UH/{print $0}' \
			"$dadger.bkp" > "$dadger"
		fi
	done
...
if [ -d $ano$mes ]; then
    ...
    if [ -f "cortes.dat" ]; then
        ...
        echo /home/compass/sacompass/previsaopld/cpas_ctl_common/scripts/nwlistop27.sh
        /home/compass/sacompass/previsaopld/cpas_ctl_common/scripts/nwlistop27.sh
        ...
        echo /home/compass/sacompass/previsaopld/cpas_ctl_common/scripts/newdesp27.sh
        /home/compass/sacompass/previsaopld/cpas_ctl_common/scripts/newdesp27.sh
       ...
       if [[ "$mesN" == "13" ]]; then
        ...
        if [ -d $anoN$mesN ]; then    
          ...
          setEARMDecompNext "$ano$mes" "$anoN$mesN"
