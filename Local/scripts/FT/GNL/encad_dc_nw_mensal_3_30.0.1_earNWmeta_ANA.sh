...
agendaDCGNL(){
    ...
    ord=$(( 19 ))
    usr="encad"
    fn="/home/producao/PrevisaoPLD/enercore_ctl_common/queue/encadeado_earmNWmeta_DC${ano}${mes}_${dt}"
    cmd="/home/producao/PrevisaoPLD/enercore_ctl_common/scripts/FT/GNL/DADGNL_decomp31.27Viab_earNWmeta_ANA_NW30.0.1.sh"
...
#  if [[ "$mes" == "01" || "$mes" == "05" || "$mes" == "09" ]]; then
  ...
  if [ ! -f "cortesh.dat" ]; then
      echo /home/producao/PrevisaoPLD/enercore_ctl_common/scripts/FT/newave30.0.1.sh 1
      /home/producao/PrevisaoPLD/enercore_ctl_common/scripts/FT/newave30.0.1.sh 1
      ec=$?
      if [[ $ec == 1 ]]; then 
          exit 1;
      fi
...
-----------------------------------------------------------------------------------------------------------------------
 ...
 999
EOF
    echo /home/producao/PrevisaoPLD/enercore_ctl_common/scripts/FT/nwlistop30.0.1.sh
    /home/producao/PrevisaoPLD/enercore_ctl_common/scripts/FT/nwlistop30.0.1.sh
    grep '^ *MEDIA' varmpuh*.out|sed "s/varmpuh\([0-9]\{3\}\).*MEDIA.\{7\}.\{$(((10#0$mes-1)*9))\}\(.\{9\}\).*/\1;\2/" > earm.uh
    rm varmpuh*.out
    if [ -d ../$anoN$mesN ]; then
      confhd=$(find ../$anoN$mesN/ -iname "confhd.dat")
      curl -s --get --data-urlencode "path=$encadPath" --data-urlencode "date=$ano$mes" "http://10.206.194.210/api/encad/encad_nwlistop"
      mv "$confhd" "../$anoN$mesN/confhd.bkp" && mv "${confhd}.earm" "$confhd"
      modif=$(find "../$anoN$mesN" -iname 'modif.dat')
      dos2unix "$modif"
      mv "${modif}" "${modif}.bkp"
      awk -v "n=$n"  -v "y=$anoN" -v "m=$mesN" -f "/home/producao/PrevisaoPLD/enercore_ctl_common/awk/2024/SF+Paranapanem+Grande+Paranaiba/modif_hibrido_nwmeta.awk" "$confhd" "${modif}.bkp" >> "${modif}" || exit 111
      ...
      for dc in $dcPaths ; do
        if [ -d "../$dc/$anoN$mesN" ]; then
          ...
          dos2unix "$dadger.earm"
          awk -v n=$n -v m=$mesN -v nn=$nn -f "/home/producao/PrevisaoPLD/enercore_ctl_common/awk/2024/SF+Paranapanem+Grande+Paranaiba/dadger_hibrido_nwmeta.awk"  "$confhd" "${dadger}.earm" > "$dadger" || exit 111
          rm "$dadger.earm"
        fi
      done
    fi
    #if [[ "$mes" == "01" ]]; then
    if [[ ! -z $first ]]; then
      cp  cortes-060.dat "../${ano}05/cortes-pos.dat"
      cp  cortes-060.dat "../${ano}09/cortes-pos.dat"
      cp  cortesh.dat "../${ano}05/cortesh-pos.dat"
      cp  cortesh.dat "../${ano}09/cortesh-pos.dat"
