...
AgendaProximaIteracao(){
    ...
    cmd="/home/producao/PrevisaoPLD/enercore_ctl_common/scripts/FT/GNL/encad_dc_nw_mensal_3_2812_earDC.sh $1"
...
' temp "../../$anoN$mesN/confhd.bkp" > "$confhd"
  ...
  for i in 0 1; do
    if [[ $(sed -n "/^ USINA *${x[$i]}/,/^ USINA/{/^ USINA/b;p}" $modif|wc -l) -eq 1 ]]; then
      if [[ $(sed -n "/^ USINA *${x[$i]}/,/^ USINA/{/^ USINA/b;p}" $modif|grep -c 'VAZMINT') > 0 ]]; then
  ...
  awk -v "n=$n"  -v "y=$anoN" -v "m=$mesN" -f "/home/producao/PrevisaoPLD/enercore_ctl_common/awk/modif_hibrido.awk" temp tmp >> "$modif"
  rm tmp45 tmp46 tmp
