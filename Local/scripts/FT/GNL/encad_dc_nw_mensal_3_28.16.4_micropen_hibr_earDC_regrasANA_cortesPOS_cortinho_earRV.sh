...
if [ -d $ano$mes ]; then
  ...
  #  fi
    if [ $(sed '99!d' dger.dat|grep -o '[0-9]'|head -1) -eq 1 ]; then
      if [ ! -f 'cortesh-pos.dat' ]; then
        if [ -f "../$ano$(printf '%02d' $(( 10#0$mes-1 )))/cortes-pos.dat" ]; then
          cp "../$ano$(printf '%02d' $(( 10#0$mes-1 )))/cortes-pos.dat" .
          cp "../$ano$(printf '%02d' $(( 10#0$mes-1 )))/cortesh-pos.dat" .
        else
          cp "../$ano$(printf '%02d' $(( 10#0$mes-1 )))/cortes-060.dat" cortes-pos.dat
          cp "../$ano$(printf '%02d' $(( 10#0$mes-1 )))/cortesh.dat" cortesh-pos.dat
        fi
      fi
    fi
    #Altera Adterm
    ...
    if [ -f "cortesh.dat" ]; then
        #echo /home/producao/PrevisaoPLD/enercore_ctl_common/scripts/nwlistop29.sh
        #/home/producao/PrevisaoPLD/enercore_ctl_common/scripts/nwlistop29.sh
        #ec=$?
        #if [[ $ec != 0 ]]; then
        #    exit $ec;
        #fi
    ...
    #  fi
        ...
        for dc in $dcPaths ; do
            if [ -d "$dc/$ano$mes" ]; then
                ...
                if [ ! "$gnl" == "" ]; then
                    agendaDCGNL "$( pwd )/$dc/$ano$mes/rv0"
                    sleep 2
                    echo ""
                fi
            fi
