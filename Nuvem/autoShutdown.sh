...
if [ "$status" = "running" ]
then  
	while [ $iddleCount -lt $tempo ] && [ "$status" != "deallocated" ]; do
		# && [ "$status" = "running" ]
		#cd $DIR;
	...
	then
		...
		/bin/bash /home/compass/sacompass/previsaopld/cpas_ctl_common/verifica_run.sh $host
	else
		rm -f "/home/compass/sacompass/previsaopld/cpas_ctl_common/loadavg/$host"
		/bin/bash /home/compass/sacompass/previsaopld/cpas_ctl_common/verifica_run.sh $host
		sleep 300
