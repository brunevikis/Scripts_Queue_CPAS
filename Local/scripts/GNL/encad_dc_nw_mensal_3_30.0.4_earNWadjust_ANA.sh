...
agendaDCGNL(){
    ...
    ord=$(( 19 ))
    usr="encad"
    fn="/home/producao/PrevisaoPLD/enercore_ctl_common/queue/encadeado_earmNWadjust_DC${ano}${mes}_${dt}"
    cmd="/home/producao/PrevisaoPLD/enercore_ctl_common/scripts/GNL/DADGNL_decomp32.0.1Viab_earNWadjust_ANA_NW30.0.4.sh"
...
if [ -d $ano$mes ]; then
  ...
  if [ -f "cortesh.dat" ]; then
    ...
    if [ -d ../$anoN$mesN ]; then
      curl -s --get --data-urlencode "path=$encadPath" --data-urlencode "date=$ano$mes" "http://10.206.194.210/api/encad/encad_nw_adjust_restr"
    fi
