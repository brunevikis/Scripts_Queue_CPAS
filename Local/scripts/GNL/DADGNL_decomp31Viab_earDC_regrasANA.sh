...
AgendaProximaIteracao(){
    ...
    cmd="/home/producao/PrevisaoPLD/enercore_ctl_common/scripts/GNL/encad_dc_nw_mensal_3_280003_earDC_regrasANA.sh $1"
...
if [ -d ../../$anoN$mesN ]; then
  ...
  mv "../../$anoN$mesN/confhd.dat" "../../$anoN$mesN/confhd.bkp"
  awk -v n="$n" 'BEGIN{FS=" +; *"}
NR==FNR{if($1==n&&$3==1)
{if($12=="-"){v[$5+0]=0}else{v[$5+0]=$12}};next}
{FIELDWIDTHS = "5 30 6 30";if(FNR==1||FNR==2){print $0}else{printf "%s%s%6.2f%s\n",$1,$2,v[$1+0],$4}}
' temp "../../$anoN$mesN/confhd.bkp" > "../../$anoN$mesN/confhd.dat"

  modif=$(find "../../$anoN$mesN" -iname 'modif.dat')
  mv "$modif" "$modif.bkp"
  awk -v "n=$n"  -v "y=$anoN" -v "m=$mesN" -f "/home/producao/PrevisaoPLD/enercore_ctl_common/awk/modif.awk" temp "$modif.bkp" >> "$modif"
  re=$(find "../../$anoN$mesN" -iname 're.dat')
  mv "$re" "$re.bkp"
  awk -v "n=$n" -v "y=$anoN" -v "m=$mesN" -f "/home/producao/PrevisaoPLD/enercore_ctl_common/awk/re.awk" temp "$re.bkp" > "$re"
...
' temp "../../$dc/$anoN$mesN/dadger.bkp" > "$dadger.tmp"
        ...
        awk -v n=$n -v m=$mesN -v nn=$nn -f "/home/producao/PrevisaoPLD/enercore_ctl_common/awk/dadger.awk"  temp "$dadger.tmp" > "$dadger"
        rm "$dadger.tmp"
      fi
    done
  rm temp
