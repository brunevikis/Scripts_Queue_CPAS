...
AgendaProximaIteracao(){
    ...
    cmd="/home/producao/PrevisaoPLD/enercore_ctl_common/scripts/FT/GNL/encad_dc_nw_mensal_3_281106_earRV.sh $1"
...
if [ -d "../../$anoN$mesN/rv0" ] && [ "$n" -eq 1 ]; then
  ...
  for dc in $dcPaths ; do
    if [ -d "../../../$dc/$anoN$mesN/rv0" ]; then
      ...
      !/^UH/{print $0}
      ' temp "../../../$dc/$anoN$mesN/rv0/dadger.bkp" > "$dadger"
    fi
  done
  #agenda proximomes
  cd ../../..
