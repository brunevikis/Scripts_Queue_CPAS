...
#Verifica erros no Newave Correspondente

dir=$(pwd)

 #find /home/compass/queuectl/finished/ -name encadeado_NW* -exec grep -H -i $(pwd) {} \; | cut -f1 -d":" | grep $ano$mes | sort -n | tail -1

Command_Caso=$(find /home/compass/queuectl/finished/ -name encadeado_NW* -exec grep -H -i $(pwd) {} \; | cut -f1 -d":" | grep $ano$mes | sort -n | tail -1)
commandname=$(basename "$Command_Caso");
		
Status_Code=$(sed -n 's/^EXITCODE=//p' $Command_Caso);
...
then
	...
	then
		echo $Status_Code "Erro no Newave Correspondente"
		commandname=$(basename "$Command_Caso");
		
		data=$(TZ=":GMT+3" date +%Y-%m-%d\ %T)
		
		echo -e "$data\n Encadeado interrompido!\n Comando:$Dir_Com\n ExitCode: $Status_Code"
		
		#echo -e "$data\n Encadeado interrompido!\n Comando:$Dir_Com\n ExitCode: $Status_Code" | mail -A cpas -s "Erro Encadeado $commandname" pedro.modesto@cpas.com.br,alex.marques@cpas.com.br,bruno.araujo@cpas.com.br,natalia.biondo@cpas.com.br,diana.lima@cpas.com.br
		
		#echo -e "$data\n Encadeado interrompido!\n Diretorio:$dir\n ExitCode: $Status_Code" | mail -A cpas -s "Erro Encadeado $commandname" alex.marques@cpas.com.br
		exit 1
	fi
else
	Command_Caso=$(find /home/compass/queuectl/finished/ -name EncadeadoMensal-GNL* -exec grep -H -i $(pwd) {} \; | cut -f1 -d":" | sort -n | tail -1)
	commandname=$(basename "$Command_Caso");		
	Status_Code=$(sed -n 's/^EXITCODE=//p' $Command_Caso);
	if [ "$Status_Code" != "" ]
	then
