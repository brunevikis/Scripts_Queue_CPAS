...
AgendaProximaIteracao(){
    ...
    cmd="/home/producao/PrevisaoPLD/enercore_ctl_common/scripts/FT/GNL/encad_dc_nw_mensal_3_28.16.4_micropen_hibr_earDC_regrasANA_cortesPOS_cortinho_earRV.sh $1"
...
AgendaProximaRV(){
    ...
    cmd="/home/producao/PrevisaoPLD/enercore_ctl_common/scripts/FT/GNL/DADGNL_decomp31.24Viab_hibr_earDC_regrasANA_cortesPOS_micropen_cortinho_earRV.sh"
...
if [ -d ../../../$anoN$mesN ]; then
  if [ -f "dec_oper_sist.csv" ]; then
    ...
    dcPaths=$( ls ../../.. | grep -E '^20[0-9]{4}' -v )
    if [ -d "../../../$anoN$mesN/rv0" ] && [ "$n" -eq 1 ]; then
...
' temp "../../../$anoN$mesN/confhd.bkp" > "$confhd"
      ...
      mv "${modif}" "${modif}.bkp"
      awk -v "n=$n"  -v "y=$anoN" -v "m=$mesN" -f "/home/producao/PrevisaoPLD/enercore_ctl_common/awk/2024/modif_hibrido.awk" temp "${modif}.bkp" >> "${modif}" || exit 111
      dcPaths=$( ls ../../.. | grep -E '^20[0-9]{4}' -v )
      for dc in $dcPaths ; do
        if [ -d "../../../$dc/$anoN$mesN/rv0" ]; then
          dadger=$(find "../../../$dc/$anoN$mesN/rv0" -iname 'dadger.rv?')
...
!/^UH/{print $0}
' temp "../../../$dc/$anoN$mesN/rv0/dadger.bkp" > "$dadger.tmp"
          awk -v n=$n -v m=$mesN -v nn=$nn -f "/home/producao/PrevisaoPLD/enercore_ctl_common/awk/2024/dadger_hibrido.awk"  temp "$dadger.tmp" > "$dadger"
          rm "$dadger.tmp"
        fi
      done
      rm temp
