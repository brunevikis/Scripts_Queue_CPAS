...
AgendaProximaIteracao(){
    ...
    cmd="/home/producao/PrevisaoPLD/enercore_ctl_common/scripts/FT/GNL/encad_dc_nw_mensal_3_28.16.4_micropen_earREENW+DC_cortesPOS_cortinho.sh $1"
...
#' temp "../../$anoN$mesN/confhd.bkp" > "$confhd"
    ...
    #mv "${modif}" "${modif}.bkp"
    #awk -v "n=$n"  -v "y=$anoN" -v "m=$mesN" -f "/home/producao/PrevisaoPLD/enercore_ctl_common/awk/2024/SF+Paranapanema/modif_hibrido.awk" temp "${modif}.bkp" >> "${modif}" || exit 111
    #re=$(find "../../$anoN$mesN" -iname 're.dat')
    #mv "$re" "$re.bkp"
    #awk -v "n=$n" -v "y=$anoN" -v "m=$mesN" -f "/home/producao/PrevisaoPLD/enercore_ctl_common/awk/2024/SF+Paranapanema/re_hibrido.awk" temp "$re.bkp" > "$re"
    dcPaths=$( ls ../.. | grep -E '^20[0-9]{4}' -v )
      for dc in $dcPaths ; do
...
' temp "../../$dc/$anoN$mesN/dadger.bkp" > "$dadger.tmp"
          #awk -v n=$n -v m=$mesN -v nn=$nn -f "/home/producao/PrevisaoPLD/enercore_ctl_common/awk/2024/SF+Paranapanema/dadger_hibrido.awk"  temp "$dadger.tmp" > "$dadger"
          #rm "$dadger.tmp"
          mv "$dadger.tmp" "$dadger"
        fi
      done
    rm temp
    #agenda proximomes
