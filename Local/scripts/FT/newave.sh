...
if [ $tipoExec -eq 1 ]; then
    ...
    if [ $tipoSim -eq 1 -o $tipoSim -eq 2 ]; then
        ...
        work_dir=~/newave/$(date +%Y%m%d%H%M%s%N)/
        ...
        { zip -q etc cortes-*.dat alertainv_00* avl_* sgterm* newave_00*.log fpha_0*.svc eng*.dat vazinat.dat parp* \
            && rm -f cortes-*.dat alertainv_00* avl_* sgterm* newave_00*.log fpha_0*.svc eng*.dat vazinat.dat parp* ; } &
        wait;
