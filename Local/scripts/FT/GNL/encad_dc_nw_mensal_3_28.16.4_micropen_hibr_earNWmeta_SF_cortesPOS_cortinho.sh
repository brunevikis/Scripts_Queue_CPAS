...
agendaDCGNL(){
    ...
    cmd="/home/producao/PrevisaoPLD/enercore_ctl_common/scripts/FT/GNL/DADGNL_decomp31.24Viab_hibr_earNWmeta_SF_cortesPOS_micropen_cortinho.sh"
...
EOF
    ...
    if [ -d ../$anoN$mesN ]; then
      ...
      mv "${modif}" "${modif}.bkp"
      awk -v "n=$n"  -v "y=$anoN" -v "m=$mesN" -f "/home/producao/PrevisaoPLD/enercore_ctl_common/awk/2024/SF+Paranapanema/modif_hibrido_nwmeta.awk" "$confhd" "${modif}.bkp" >> "${modif}" || exit 111
